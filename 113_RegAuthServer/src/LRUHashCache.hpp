...
template <typename TConcreteObj>
CHashMap_KString<unsigned>* CLRUHashCache<TConcreteObj>::m_pHashMap = NULL;
...
void CLRUHashCache<TConcreteObj>::AllocateFromShm(CSharedMemory& shm, bool bResume)
{
    ...
    else
    {
        ...
        //todo jasonxiong 目前不需要支持从共享内存中恢复
        /*
    ...
    //todo jasonxiong 目前不支持从共享内存中恢复的模式
    /*
    if (bResume)
    {
        // 恢复使用中的obj
...
void CLRUHashCache<TConcreteObj>::SwapOut(const int iNodeNumber)
{
    ...
    while (i < iNodeNumber)
    {
        ...
        const TDataString& stKey = pObj->GetKey();
...
template <typename TConcreteObj>
TConcreteObj* CLRUHashCache<TConcreteObj>::CreateByKey(const char* pszKey)
{
    if(!pszKey)
    {
...
template <typename TConcreteObj>
int CLRUHashCache<TConcreteObj>::DeleteByKey(const TDataString& stKey)
{
    unsigned iObjID = 0;
...
template <typename TConcreteObj>
TConcreteObj* CLRUHashCache<TConcreteObj>::GetByKey(const TDataString& stKey)
{
    unsigned iObjID = 0;
...
template <typename TConcreteObj>
TConcreteObj* CLRUHashCache<TConcreteObj>::GetByKey(const char* pszKey)
{
    if(!pszKey)
    {
        TRACESVR("Failed to get by key in LRUHashCache, invalid param!\n");
        return NULL;
    }

    TDataString stKey(pszKey);

    return GetByKey(stKey);
}
