...
public:
    ...
    static void AllocateFromShm(CSharedMemory& shm);
...
template <typename TConcreteObj>
void CLRUHashCache<TConcreteObj>::AllocateFromShm(CSharedMemory& shm)
{
    ...
    m_pBiDirQueue = CBiDirQueue::CreateByGivenMemory(
                        (char*)shm.GetFreeMemoryAddress(), shm.GetFreeMemorySize(),
                        GetQueueNodeNumber());
    assert(m_pBiDirQueue);
    shm.UseShmBlock(CaculateQueueSegSize());

    m_pAllocator->Initialize();
    m_pBiDirQueue->Initialize();
}
