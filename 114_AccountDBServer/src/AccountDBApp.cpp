...
int CAccountDBApp::ms_iAppCmd = APPCMD_NOTHING_TODO;
CAccountDBInfoConfig CAccountDBApp::m_stAccountDBConfigManager;
CAccountDBInfoConfig CAccountDBApp::m_stUniqUinDBConfigManager;

CAccountDBApp::CAccountDBApp()
{
    m_bResumeMode = false;
}
...
CSSProtocolEngine* CAccountDBApp::GetProtocolEngine(int iThreadIdx)
{
    return m_stThreadManager.GetThread(iThreadIdx)->GetProtocolEngine();
}

int CAccountDBApp::Initialize(bool bResume, int iWorldID)
{
    int iRes = 0;
...
void CAccountDBApp::Run()
{
    TRACESVR("AccountDBServer starts running ...\n");
...
int CAccountDBApp::HandleMsgIn(int& riRecvMsgCount)
{
...
int CAccountDBApp::ReceiveAndDispatchMsg()
{
    ...
    // 接受网络数据
    int iRet = m_stMsgTransceiver.RecvOneMsg(m_szCodeBuf, iBufLen, iCodeLen, GAME_SERVER_REGAUTH);
...
void CAccountDBApp::SetAppCmd(int iAppCmd)
{
    ms_iAppCmd = iAppCmd;
}

int CAccountDBApp::LoadLogConfig(const char* pszConfigFile, const char* pszLogName)
{
    char szLogPath[64] = {0};
    char szLogName[64] = {0};
    int iLogLevel;
    ...
    //初始化log
    int iRt = m_stLogManager.Initialize(CAccountDBThreadManager::THREAD_NUMER, stServerLogConfig);
    if (iRt != 0)
    {
        return -5;
    }
...
int CAccountDBApp::ReloadConfig()
{
    LoadLogConfig(APP_CONFIG_FILE, "AccountDBServer");

    //load accountDB config
    int iRet = m_stAccountDBConfigManager.LoadAccountDBInfoConfig(ACCOUNTDBINFO_CONFIG_FILE);
    if(iRet)
    {
    ...
    //load UniqUinDB config
    iRet = m_stUniqUinDBConfigManager.LoadAccountDBInfoConfig(UNIQUINDBINFO_CONFIG_FILE);
    if(iRet)
    {
...
int CAccountDBApp::HandleThreadMsg(int& riSendMsgCount)
{
    int iRet = 0;
    ...
    int i = 0;
    for (i = 0; i < CAccountDBThreadManager::THREAD_NUMER; i++)
    {
        iRet = m_stThreadManager.PopCode(i, (unsigned char*)m_szCodeBuf, iMaxLength, riMsgLength);
        if ((iRet >= 0) && (riMsgLength > 0))
        {
            // 发送网络数据
            int iRet = m_stMsgTransceiver.SendOneMsg(m_szCodeBuf, riMsgLength, GAME_SERVER_REGAUTH);
            if (iRet < 0)
            {
