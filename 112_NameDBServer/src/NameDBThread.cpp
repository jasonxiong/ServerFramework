...
void* ThreadProc( void *pvArgs )
{
	...
	CNameDBThread *pThread = (CNameDBThread *)pvArgs;
...
int CNameDBThread::Initialize(bool bResume, const int iThreadIdx)
{
	int iRt = 0;
	...
	iRt = m_stNameDBHandlerSet.Initialize(iThreadIdx);
	if (iRt != 0)
	{
		return -9;
	}
...
int CNameDBThread::CreateThread()
{
	int iRt = 0;
...
int CNameDBThread::InitCodeQueue(bool bResume, const int iThreadIdx)
{
...
int CNameDBThread::ReceiveMsg(int& riCodeLength)
{
	int iRet = 0;
...
int CNameDBThread::ProcessMsg(SHandleResult& stMsgHandleResult)
{
	//process the msg
	CHandler* pHandler = m_stNameDBHandlerSet.GetHandler(m_stGameMsg.m_stmsghead().m_uimsgid());
	if (!pHandler)
	{
...
int CNameDBThread::EncodeAndSendMsg(SHandleResult& stHandleResult)
{
...
int CNameDBThread::PushCode(const unsigned char* pMsg, int iCodeLength)
{
	if (!pMsg)
	{
		return -1;
...
int CNameDBThread::PopCode(unsigned char* pMsg, int iMaxLength, int& riLength)
{
	if (!pMsg)
	{
		return -1;
